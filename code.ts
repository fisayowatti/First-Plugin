...
const lightenHex = (hexColor, alpha: number) => {
...
const darkenHex = (hexColor, alpha: number) => {
  const rgb = hexToRGBA(hexColor);
...
const glassify = (node: SceneNode, lightIntensity: number, lightColor: string, bgColor: string) => {
